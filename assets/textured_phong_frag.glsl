...
void main() {
	vec3 textureColor = texture2D(mat.texture, textureCoord).xyz;
    ...
    vec3 amb = materialAmb * lightColor / 10.0 * textureColor;
    vec3 diff = vec3(0);
    vec3 spec = vec3(0);
    if (inSpotlight) {
    	diff = clamp(dot(N, L), 0., 1.) * materialDiff * textureColor * mat.Kd / sqrt(dist + 1.0);
    	spec = pow(clamp(-dot(V, R), 0., 1.), shininess) * textureColor * lightColor * mat.Ks / sqrt(dist + 1.0);
    }
    
    gl_FragColor = vec4((amb + diff + spec) * 3., 1);
}