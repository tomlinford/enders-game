...
void main() {
    ...
    vec3 diff = clamp(dot(N, L), 0., 1.) * materialDiff * lightColor * textureColor;// / sqrt(dist + 1.0);
    vec3 spec = pow(clamp(-dot(V, R), 0., 1.), shininess) * materialSpec * lightColor * textureColor;// / sqrt(dist + 1);
